...
@bot.tree.command(name="set_nick_lock_roles", description="Вказати ролі, яким заборонено змінювати власний нікнейм")
@app_commands.describe(roles="Ролі для блокування зміни ніку")
async def set_nick_lock_roles(interaction: discord.Interaction, roles: list[discord.Role]):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message("❌ Потрібні права адміністратора", ephemeral=True)
    guild_id = str(interaction.guild.id)
    nick_lock_roles[guild_id] = [role.id for role in roles]
    save_nick_lock_roles()
    await interaction.response.send_message(
        f"✅ Заборонено змінювати нікнейм для ролей: {', '.join(role.mention for role in roles)}",
        ephemeral=True
    )
...
async def on_member_update(before, after):
    ...
    if before.nick != after.nick:
        ...
        locked_roles = nick_lock_roles.get(guild_id, [])
        # Якщо у користувача є хоч одна роль із заборонених
        if any(role.id in locked_roles for role in after.roles):
            user_id = str(after.id)
            now = datetime.utcnow()
            entry = nick_flood_counter.get(user_id, {'count': 0, 'last': now, 'muted': 0})
            # Якщо минуло більше 24 годин — скидаємо лічильник
            if (now - entry['last']) > timedelta(hours=24):
                entry = {'count': 0, 'last': now, 'muted': 0}
            entry['count'] += 1
            entry['last'] = now
            nick_flood_counter[user_id] = entry
            # Повертаємо старий нік
            try:
                await after.edit(nick=before.nick)
            except Exception as e:
                print(f"[ERROR] Не вдалося повернути нік {after}: {e}")
            # DM користувачу
            try:
                msg = "❌ Вам заборонено змінювати нікнейм на цьому сервері."
                if entry['count'] == 3 and entry['muted'] < 1:
                    # Мут на 10 хвилин
                    until = discord.utils.utcnow() + timedelta(minutes=10)
                    await after.edit(timed_out_until=until, reason="Flood зміни ніку (10 хв)")
                    entry['muted'] = 1
                    msg += "\nВи отримали мут на 10 хвилин за багаторазову спробу зміни ніку."
                elif entry['count'] > 3 and entry['muted'] < 2:
                    # Мут на 24 години
                    until = discord.utils.utcnow() + timedelta(hours=24)
                    await after.edit(timed_out_until=until, reason="Flood зміни ніку (24 години)")
                    entry['muted'] = 2
                    msg += "\nВи отримали мут на 24 години за повторний flood зміни ніку."
                await after.send(msg)
            except Exception as e:
                print(f"[ERROR] Не вдалося надіслати DM {after}: {e}")
            # Повідомляємо у канал для логів (якщо налаштовано)
            guild_notify_channel_id = nick_notify_channel.get(guild_id)
            notify_channel = after.guild.get_channel(guild_notify_channel_id) if guild_notify_channel_id else None
            if notify_channel:
                await notify_channel.send(f"⛔ {after.mention} намагався змінити нік, але це заборонено для його ролі. Спроба: {entry['count']}")
